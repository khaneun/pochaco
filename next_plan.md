...
## Phase 2 — 종목 유니버스 메이저 제한 🔵 백테스트 측정 완료 (2026-06-15)
...
### 측정 방식 (2026-06-15 확정·구현)
baseline.py(운영 trades 주입·청산만 시뮬)로는 종목 선정 변경을 측정 불가 →
**룰 기반 진입 엔진** 신규 구축(LLM 6에이전트 → "필터+스코어 상위 N" 결정론 룰 대체):
- `backtest/indicators.py` (5분봉 지표 재계산, 순수 파이썬) +
  `backtest/rule_engine.py` (walk-forward 진입·동시보유 8·쿨다운·동적 블랙리스트)
- `scenarios/rule_baseline.py`(풀 유니버스+변동성보상ON) vs `scenarios/phase2.py`(메이저+보상제거+블랙리스트)
- `cli.py --scenario compare` — 두 시나리오 KPI 나란히+개선폭. 매일 timer가 baseline 검증과 함께 텔레그램 보고
- 동일 지표 공유 → 재계산 절대오차 상쇄, 상대 비교가 본질
...
**결론: 메이저 제한은 거래수↓로 총손실은 줄이나 건당 기대값은 여전히 음수(두 거래소 일관).**
Phase 2 단독으로 음의 기대값 구조 미해소 → **Phase 4(손익비 역전)가 실제 레버**임을 백테스트로 확인.
⚠️ phase2 표본 과소(단일 레짐) → 캔들 누적 후 재평가. compare는 매일 자동 누적 중.
