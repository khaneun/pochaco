...
dashboard/
  ...
  web_server.py          # HTTP 웹 대시보드 (종합 + 전문가 실적표 2페이지)
                         # API: /api/status, /api/experts, /api/liquidate
                         #      /api/agent/prompt, /api/agent/chat, /api/agent/update_prompt
deploy/
  deploy.sh              # EC2 rsync 배포 스크립트 (venv 재생성 포함)
...
# 대시보드 연동
coordinator.get_agent_prompt(role)                  # base_prompt + feedback_prompt 반환
coordinator.chat_with_agent(role, message, history) # 멀티턴 대화 (history: list[dict])
coordinator.update_agent_prompt(role, new_prompt)   # base_prompt 즉시 업데이트
```
...
class BaseLLMProvider(ABC):
    ...
    @abstractmethod
    def chat_with_system(self, system: str, messages: list[dict], max_tokens: int = 1024) -> str: ...
    # messages 형식: [{"role": "user", "content": "..."}, {"role": "assistant", "content": "..."}]
    # Anthropic: system= 파라미터 + messages 배열
    # OpenAI: system role 메시지 prepend
...
## 매매 전략 핵심 수치 (trading_engine.py) — v4.4

| 파라미터 | 범위 |
|---------|------|
| 1차 분할 익절 | +1.0% (50% 매도) |
| 2차 분할 익절 | +2.5% (30% 매도) |
| 최종 손절 | -1.5% 단일 손절 (분할 손절 폐지) |
| 익절 진입 (TP) | optimizer 결정 (3.0~4.5%) 도달 시 트레일링 |
| 트레일링 타임아웃 | 30분 |
| AI 동적 조정 간격 | 30분 |
| **최대 보유 시간** | **6시간** (v4.4: 12h → 6h) |
| **3시간 무수익 조기청산** | peak < +1.0% 시 즉시 종료 |
| **신규 진입 최소 간격** | 60분 (v4.4) |
| **단일코인 진입금 상한** | 총자산 × 8% (v4.4) |
| **투자비율 클램프** | 0.55 ~ 0.75 (v4.4) |

## 쿨다운/블랙리스트 (cooldown.py) — v4.4

| 유형 | 차단 기간 |
|------|----------|
| take_profit 후 동일 코인 | 6시간 (v4.4: 30분 → 6h) |
| stop_loss 후 동일 코인 | 12시간 |
| manual (수동 청산) | 60분 |
| 7일 손실 3회 이상 우세 | 5일 |
| 손실 2회 + 폭락(-2%↓) 1회 | 3일 |

## 거래소별 임계값 (coin_selector.py) — v4.4
...
```bash
# 권장: deploy.sh 사용 (venv 재생성 + 패키지 설치 + 서비스 재시작 자동화)
EC2_HOST=ec2-user@<IP> SSH_KEY=~/kitty-key.pem bash deploy/deploy.sh

# 수동 rsync 시 .venv/ 반드시 exclude
rsync -avz --delete \
  --exclude='.venv/' --exclude='.env' --exclude='*.db' \
  --exclude='__pycache__/' --exclude='*.pyc' \
  -e "ssh -i ~/kitty-key.pem" \
  ./ ec2-user@<IP>:/opt/pochaco/
```

**EC2 주의사항:**
- EC2 기본 Python은 3.9 — `.venv`는 반드시 `python3.12 -m venv .venv`로 생성
  (`dict | None` 등 3.10+ 타입 힌트가 코드 전반에 사용됨)
- 서비스: `sudo systemctl restart pochaco` / 로그: `sudo journalctl -u pochaco -f`
- `.venv` 없으면 deploy.sh가 자동 재생성, 직접 rsync 시 `--exclude='.venv/'` 누락하면 venv 삭제됨
...
- **파라미터 범위 준수** — TP/SL 수정 시 optimizer의 clamp 범위(tp: 4~10%, sl1: -1.5% 고정, sl2: -1.7~-2.0%) 내에서 변경
