...
## v5 Phase2 (2026-06-15)

### feat: 룰 기반 진입 엔진 + 시나리오 비교 (메이저 유니버스 제한)

#### 배경
6/14 1차 백테스트로 baseline 엔진 신뢰도 확인(실현손익 ±5% 일치 97.4%) → Phase 2 진입.
baseline은 운영 trades를 주입해 청산만 시뮬하므로 **종목 선정 변경(Phase 2의 본질)을
측정할 수 없음** → LLM 6에이전트를 결정론 룰로 대체하는 진입 엔진을 신규 구축.

#### 핵심 변경 (`backtest/`)
- `indicators.py` — 5분봉에서 거래대금·변동폭·모멘텀·RSI·변화율 재계산 (순수 파이썬, numpy 미사용)
- `rule_engine.py` — walk-forward 진입 생성: coin_selector 필터/스코어 근사 + 동시보유 8·
  신규진입 60분 간격·쿨다운·(동적)손실 블랙리스트. 시나리오 차이는 `RuleConfig`로만 표현
- `scenarios/rule_baseline.py` — 풀 유니버스 + 변동성 보상 ON + 블랙리스트 OFF (현 v4.4 근사)
- `scenarios/phase2.py` — 메이저 화이트리스트(~27종) + 변동성 보상 제거 + 손실 3회 영구 차단
- `cli.py` — `--scenario rule_baseline|phase2|compare` 추가. compare는 두 시나리오 KPI 나란히 + 개선폭
- `deploy/{pochaco,kuromi}-backtest.service` — 매일 compare 보고를 baseline 검증과 함께 텔레그램 발송

#### 1차 결과 (2026-06-15, 두 거래소)
| | rule_baseline | phase2 |
|---|---|---|
| 빗썸 | 203건 EV -0.18%/건 누적 -37%p | 26건 EV -0.26%/건 누적 -7%p |
| 업비트 | 328건 EV -0.20%/건 누적 -66%p | 37건 EV -0.62%/건 누적 -23%p |

- **메이저 제한은 거래수를 줄여 총 손실은 감소하나, 건당 기대값은 여전히 음수**(두 거래소 일관)
  → Phase 2 단독으로는 음의 기대값 구조가 해소되지 않음. **Phase 4(손익비 역전)가 실제 레버**임을 확인
- ⚠️ 한계: phase2 표본이 단일 레짐(메이저는 변동성 floor 통과 구간에만 진입)으로 과소 →
  캔들 누적 후 재평가 필요. 상대 비교는 동일 지표 공유로 견고(절대값보다 상대 비교가 본질)

---

## v4.4.2 (2026-06-01)

### fix: 업비트 상장폐지 마켓으로 인한 ticker 배치 조회 404 차단 (kuromi)

#### 배경
kuromi(업비트) 운영 중 시장 분석 사이클마다 ticker 배치 조회가 404로 실패하는 장애 발생.
- `KRW-DRIFT`가 업비트에서 **상장폐지**("Code not found")됐으나 봇의 마켓 목록 캐시에 잔존
- `get_ticker("ALL")`이 100개 단위 배치 요청 시 DRIFT 하나가 섞여 **배치 전체가 404**
- kuromi가 5/23부터 무재시작 상태라 캐시가 갱신되지 않아 영구 지속

#### 근본 원인 (2중 결함)
1. **`_get_krw_markets()`가 TTL 없는 세션 영구 캐시** (`upbit_client.py`) — 장기 무재시작 시 상장폐지가 반영되지 않음
2. **`get_ticker("ALL")` 배치 루프가 `raise_for_status()`로 전체 실패** — 배치 내 폐지 마켓 1개가 전체 조회를 깨뜨림

#### 핵심 변경 (`core/upbit_client.py`)
- **마켓 캐시 1시간 TTL 추가** (`_MARKETS_CACHE_TTL_SEC = 3600`): `_get_krw_markets()`가 주기적으로 `/v1/market/all` 재로드 → 폐지 코인 선제 제거
- **배치 404 시 캐시 무효화 후 폴백**: `get_ticker("ALL")` 배치 실패 시 `_markets_cache = None`으로 무효화 → 다음 조회에서 최신 목록 재로드(자가 치유)

#### 효과
- 상장폐지 코인 발생 시에도 ticker 배치 조회 정상화 (404 0건)
- 빗썸/업비트 공용 코드베이스이므로 양 봇 모두 적용

---

## v4.4.1 (2026-06-01)

### fix: 포트폴리오 종료 시 dust 잔량 매도 무한루프 차단

#### 배경
운영 중 `매화-87` 포트폴리오의 VTHO 코인이 **약 12시간(5/31 16:06~6/1 04:11) 매도 무한 재시도 루프**에 빠진 장애 발생.
- 분할 익절 후 남은 VTHO 잔여분 평가금액이 **3,795원**(5,000원 미만 dust)으로 하락
- 빗썸이 `under_min_total_ask`("최소주문금액 5000원 이상")로 매도 주문을 거부
- 봇은 "매도 완전 실패 → 포지션 유지 → 다음 루프 재시도"로 처리 → 시세가 오르지 않는 한 영구 탈출 불가
- 6시간 타임아웃 강제매도가 매 루프 트리거되지만 같은 dust 벽에 막혀 포트폴리오가 종료되지 못하고 감시 슬롯 점유

#### 근본 원인
분할 매도 함수(`_split_sell`)에는 dust 가드가 있었으나(`trading_engine.py:1201`, `1208` — 잔여분 < MIN_ORDER_KRW → 전량 매도 전환), **포트폴리오 전량 매도 종료 함수 `_execute_portfolio_sell`에는 해당 가드가 부재**. `actual_units <= 0`만 체크하고 "수량은 있으나 평가금액이 5,000원 미만"인 dust 케이스를 거르지 못해 무조건 매도를 시도 → 무한 재시도.

#### 핵심 변경
**dust 가드 추가 (`trading_engine.py` `_execute_portfolio_sell`)**
- 목표 매도가 결정을 dust 판정과 매도 주문이 공용하도록 루프 상단으로 이동
- `actual_units × 현재가 < MIN_ORDER_KRW`이면 매도 시도 없이 dust로 간주
- 기존 `actual_units <= 0` 분기와 통합 처리 — Trade 테이블 기준 손익 집계 후 `close_position` 호출 (dust 잔량은 빗썸 지갑 잔존)

#### 효과
- dust 잔량 발생 시에도 포트폴리오 정상 종료 (무한루프 탈출)
- 감시 슬롯 점유·로그 폭주 해소
