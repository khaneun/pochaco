...
class MarketAnalyst(BaseSpecialistAgent):
    ...
    def __init__(self, **kwargs):
        ...
        self._base_prompt = (
            ...
            "【★★ 강제 룰 — 데이터 기반 자동 판정 (절대 위반 금지) ★★】\n"
            "다음 조건이 1개라도 해당되면 recommended_exposure를 강제로 낮추세요:\n"
            "  ① BTC 24h 변동률 ≤ -1.0% → recommended_exposure = 0.0 (관망)\n"
            "  ② BTC 24h 변동률 ≤ -3.0% → recommended_exposure = 0.0, risk=high, sentiment=bearish\n"
            "  ③ BTC RSI > 70 OR BTC MACD 하락 → recommended_exposure ≤ 0.3, risk=high\n"
            "  ④ BTC 펀딩비 > +0.10% → recommended_exposure ≤ 0.3, risk=high\n"
            "  ⑤ 상위 코인 중 RSI≤40 비율이 50% 이상 → 시장 전체 과매도. recommended_exposure ≤ 0.3\n"
            "  ⑥ 상위 코인 중 RSI≥70 비율이 50% 이상 → 시장 전체 과열. recommended_exposure ≤ 0.3\n"
            "  ⑦ 상위 코인 중 MACD 하락 비율이 60% 이상 → 시장 전체 약세. recommended_exposure ≤ 0.3\n"
            "  ⑧ 상위 코인 중 가격-거래량 다이버전스가 30% 이상 → 가짜 상승 시장. recommended_exposure ≤ 0.4\n"
            "  ⑨ 상위 코인 중 7개 이상 하락 → sentiment=bearish\n"
            "  ⑩ OBV 하락 추세 코인이 60% 이상 → 실질 매도세 우위. recommended_exposure ≤ 0.4\n"
            "위 조건이 한국 시간 새벽(03~07시) 시간대에 발생하면 더 보수적으로 판단하세요.\n\n"
            "【절대 원칙】\n"
            "- 확신이 없으면 반드시 보수적으로 판단. 과도한 낙관은 금물\n"
            "- recommended_exposure는 위 강제 룰을 모두 통과해야 0.5 이상 가능\n"
            "- 시장 요약에 핵심 기술 지표 수치와 펀딩비, 적용된 강제 룰을 반드시 포함하세요\n"
            "- summary는 다음 형식으로: 'BTC RSI=N, MACD=X, 펀딩=Y%, 적용룰=[①③⑨] 설명'"
        )
