...
| 🎯 종목 사전 필터링 | CoinSelector — RSI·MACD·OBV·BB 기술 지표 + 펀딩비 기반 스코어링 → 상위 20개 후보 → AI에 전달 |
...
| 🧬 코인 특성 학습 | CoinProfileAnalyst — 매매마다 코인별 프로파일 누적·갱신 (기술 지표 중심). 매수 전 이력 코인 자동 주입 |
| 📡 파생상품 데이터 | Binance Futures / Bybit Public API — 펀딩비(8h)·미결제약정·OI 변화율. 극단 롱과열 진입 자동 차단 |
| 📊 LLM 사용량 추적 | `/system` 페이지 — Agent별 Input/Output 토큰, USD/KRW 비용 실시간 집계 |
| 📊 터미널 대시보드 | Rich Live 기반 실시간 자산·포트폴리오·성과 패널 |
| 🌐 웹 대시보드 | 종합 대시보드 + 전문가 실적표 + 시스템 현황 탭. 각 전문가 카드에서 프롬프트 확인·수정, Agent와 직접 대화 가능 |
...
│   ├── llm_provider.py      # LLM 추상화 (Anthropic / OpenAI / Gemini) + 사용량 추적
│   ├── derivatives_client.py # Binance Futures / Bybit 파생 데이터 (펀딩비·OI)
...
│   ├── technical_analyzer.py # RSI/MACD/MA/BB/OBV 기술 지표 계산 (TechnicalIndicators)
│   ├── market_analyzer.py   # 시장 데이터 수집·가공 (CoinSnapshot + DerivativesData 통합)
