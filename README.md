...
| 🔄 연속 매매 사이클 | AI 선정 → 전액 매수 → 익절/손절/시간초과 → 평가 → 반복 |
...
│   └── trading_engine.py    # 매매 루프 엔진 (현금화→선정→매수→감시→평가)
├── scheduler/
...
└──────────────────────────┬──────────────────────────────────┘
               ...
               ┌───────────▼────────────┐
               │   AI 시장 분석          │
               │  거래대금 상위 30개 코인 │  ← 과거 성과 통계 주입
               │  현재가·등락률·캔들 수집  │    (승률·추세·최근 거래 코인)
               └───────────┬────────────┘
                           │
               ┌───────────▼────────────┐
               │   LLM 코인 선정         │
               │  - 상승 모멘텀 코인 선정  │
               │  - 익절/손절% 결정       │  ← 적응형 clamp 범위 적용
               │  - R:R 2:1 이상 (유연)  │
               └───────────┬────────────┘
                           │
               ┌───────────▼────────────┐
