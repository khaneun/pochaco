...
| 🧠 AI 코인 선정 | 검증된 후보풀에서 1개 선정 + 익절/손절% 결정 |
| 🔄 연속 매매 사이클 | AI 선정 → 전액 매수 → 스마트 익절/손절 → 평가 → 반복 |
| 📈 자기 개선형 Agent | 매매 후 AI 성과 평가 → 다음 선정에 과거 통계 반영 → 전략 파라미터 자동 진화 |
| 🎣 스마트 매도 | 익절 돌파 시 트레일링 추적(오버익절 포착), 손절 터치 시 관찰 후 판단(가짜 하락 필터) |
| ⚡ 전략 최적화 | StrategyOptimizer — 매매 즉시 파라미터 재결정, 낮은 익절+넓은 손절 전략 |
| 🔧 동적 전략 조정 | 보유 중 30분 간격으로 AI가 익절/손절 기준을 재평가·조정 |
| 📊 터미널 대시보드 | Rich Live 기반 실시간 자산·포지션·차트·AI 보고서·성과 평가 패널 |
| 🌐 웹 대시보드 | 브라우저 접속 HTML 페이지 + JSON API (평가 섹션 포함) |
