...
| 기능 | 설명 |
|------|------|
| 🤖 6개 전문가 Agent | 시장 분석가·자산 운용가·매수 전문가·매도 전문가·포트폴리오 평가가·총괄 평가가 분업 체계 |
| 📊 총괄 평가 시스템 | 6시간 주기(0·6·12·18시) 전문가별 0~100 점수화 + 피드백 → 프롬프트에 동적 주입 (지속 개선) |
| 🎯 종목 사전 필터링 | CoinSelector — 변동폭·거래량·모멘텀 기반 스코어링 → 상위 10개만 AI에 전달 |
| 🧠 3단계 매수 파이프라인 | 시장 분석 → 자산 배분(투자 비율 동적 결정) → 코인 선정·TP/SL 결정 |
| 🔄 연속 매매 사이클 | AI 선정 → 동적 배분 매수 → 2단계 손절/트레일링 익절 → 평가 → 반복 |
| 📈 자기 개선형 Agent | 매매 후 AI 성과 평가 → suggested 가중평균 → 다음 선정에 즉시 반영 + 6시간 총괄 피드백 |
...
| 📊 터미널 대시보드 | Rich Live 기반 실시간 자산·포지션·차트·AI 보고서·성과 평가 패널 (일별 손익(원)/수익률 분리 표시) |
| 🌐 웹 대시보드 | 브라우저 접속 HTML 페이지 + JSON API + 포지션 청산 버튼 (일별 성과: 시작자산·종료자산·손익·수수료·승패·합계) |
...
├── strategy/
│   ├── agents/              # 6개 전문가 Agent
│   │   ├── base_agent.py    # BaseSpecialistAgent 추상 기반
...
│   │   ├── portfolio_evaluator.py # 포트폴리오 평가가 (매매 성과 분석)
│   │   └── meta_evaluator.py  # 총괄 평가가 (6시간 주기 전문가 평가·피드백)
│   ├── agent_coordinator.py # AgentCoordinator — 6개 전문가 오케스트레이션
│   ├── ai_agent.py          # AI dataclass 정의 (AgentDecision, TradeEvaluation)
...
├── database/
│   ├── models.py            # ORM: Trade / Position / DailyReport / StrategyEvaluation / AgentScore / AgentDecisionLog
│   ├── repository.py        # CRUD 레이어 (평가·전문가 점수·의사결정 로그 포함)
